<div class="content-wrapper">
    <h3 class="page-heading mb-4">Edit Page &nbsp;&nbsp;<a href="<?php echo site_url('admin/pages/add');?>" class="btn btn-primary"><i class="fa fa-plus"></i> &nbsp;Add New Page</a></h3>
               ...
               <div class="card-body my-card">
                    <div class="col-md-12 no-padding">
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control p-input" value="<?php echo $page[0]->title;?>" name="title" id="title" placeholder="Enter page title">
                        </div>
                    </div>
                </div>
                ...
                <div class="card-body my-card card-spacer">
                    <div class="col-md-12 no-padding">
                        <div class="form-group">
                            ...
                            <textarea class="form-control p-input" name="content" id="content" rows="5" placeholder="Please type the page content"><?php echo $page[0]->content;?></textarea>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="card-body my-card ">
                    <div class="col-md-12 no-padding">
                        <div class="form-group">
                            ...
                            <p><b>Author:</b> <?php echo $page[0]->first_name; ?> <?php echo $page[0]->last_name; ?></p>
                            <?php echo ($page[0]->status == 'publish') ? '<p><b>Published:</b> '. formatDate($page[0]->date_published).' at '. formatTime($page[0]->date_published) .'</p>' : '<p><b>Date Created:</b> '. formatDate($page[0]->date_created).' at '. formatTime($page[0]->date_created) .'</p>';?>
                            
                            <p>
                                <b>Status:</b> &nbsp;
                                <select id="status" class="form-control" style="display: inline;width: 250px;">
                                    <option value='publish' <?php echo ($page[0]->status == 'publish') ? 'selected':'';?>>Publish</option>
                                    <option value='draft' <?php echo ($page[0]->status == 'draft') ? 'selected':'';?>>Draft</option>
                                    <option value='trash' <?php echo ($page[0]->status == 'trash') ? 'selected':'';?>>Trash</option>
                                </select>
                                ...
                                <div id="image-styling" style="border: 2px dashed #ccc; padding: 10px;overflow: hidden;" class="col-sm-12">
                                    <center>
                                        <div id="preview">
                                            ...
                                            <?php else:?>
                                                 <div class="center-cropped"><img src="<?php echo $page[0]->image;?>" class="img-responsive"></div>
                                            <?php endif;?>  
                                        </div>
                                    </center> 
                                </div>
                                                                      
                                <input type="hidden" id="image-url" value="<?php echo $page[0]->image;?>"/>
                                <input type="hidden" id="thumb-url" value="<?php echo $page[0]->image_thumb;?>"/>
                                <br>
                               <!--dynamic form upload-->
                               
                                <form id="imageform" method="post" enctype="multipart/form-data" action='<?php echo site_url('page/admin/page/upload_image');?>'>
	...
	$(document).ready(function(){
        ...
        */
        function savePage(){
            ...
            $.ajax({
                type: "POST",
                url: base_url +'page/admin/page/update_page',
                dataType: 'json',
                data: {
                    title: title, 
                    content: content, 
        ...
        */
       	function showSuccessMessage() {
            swal({
                title: "Success!",
                text: "Page has been successfully updated.",
                type: "success",
